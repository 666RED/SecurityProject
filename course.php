    ...
    <div class="w3-container w3-margin-top">
      ...
      <table class="course-table">
        ...
        <tr>
          ...
          <td>
            <button class="w3-red delete-button" style="border-radius: 10px; padding: 5px; margin: 0"
              data-matric="<?php echo htmlspecialchars($student_matric_number); ?>"
              data-course="<?php echo htmlspecialchars($course['course_code']); ?>"
              data-section="<?php echo htmlspecialchars($course['section_number']); ?>">
              Remove
            </button>
          </td>
        </tr>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    deleteButtons.forEach(function(button) {
      button.addEventListener('click', function() {
        var studentMatricNumber = button.getAttribute('data-matric');
        var courseCode = button.getAttribute('data-course');
        var sectionNumber = button.getAttribute('data-section');

        var confirmMessage = "Remove the section?";
        if (window.confirm(confirmMessage)) {
          // Disable the button to prevent double-click
          button.disabled = true;

          var xhr = new XMLHttpRequest();
          xhr.open('POST', 'delete_section.php', true);
          xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
          xhr.onload = function() {
            if (xhr.status === 200) {
              console.log('Section removed successfully');
              window.alert('Section removed successfully');
          ...
          xhr.send('matric_number=' + encodeURIComponent(studentMatricNumber) +
            '&course_code=' + encodeURIComponent(courseCode) +
            '&section_number=' + encodeURIComponent(sectionNumber));
        }
      });
    });
  });
